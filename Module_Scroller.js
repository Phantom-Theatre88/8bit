const Module_Scroller = (() => {
  ...
  const css = `
    ...
    .scr-knob{position:absolute;left:1px;width:43px;height:50px;background:linear-gradient(#ededed,#9c9c9c);border:3px solid #444;border-radius:4px;box-shadow:0 5px 10px #000b;display:flex;align-items:center;z-index:3}
    .scr-knob:after{content:'';width:100%;height:3px;background:#ff3d00}
    ...
    .scr-deck{position:relative;padding:9px 12px;display:grid;grid-template-columns:145px minmax(0,1fr);gap:10px;align-items:stretch}
    .scr-deck.red{border-color:#763232}.scr-deck.green{border-color:#376c3a}
    ...
    .scr-film{position:absolute;top:0;height:100%;display:flex;align-items:stretch;will-change:transform;transition:transform .08s linear}
    ...
    .scr-ref{background:linear-gradient(90deg,rgba(255,255,255,.92),rgba(225,239,246,.72));color:#111;text-shadow:none}.scr-seal{position:absolute;left:50%;top:11%;bottom:11%;width:25%;transform:translateX(-50%);background:#151515;border:1px solid #555;box-shadow:0 0 0 2px rgba(255,255,255,.2);display:flex;align-items:center;justify-content:center;color:#ff5757;font:900 9px 'Roboto Mono',monospace;writing-mode:vertical-rl}.scr-ref-label{position:absolute;left:4px;bottom:4px;font-size:8px;color:#333}
  ...
  function buildFilm(id, count){
    ...
    const refLabel = count===2 ? 'A / B' : '1–24';
    return `
      <div class="scr-film" id="scr-film-${id}">
        <div class="scr-seg scr-ref" data-kind="pre"><div class="scr-seal">START</div><span class="scr-ref-label">透明 / ${refLabel}</span></div>
        ${colors.map((c,i)=>`<div class="scr-seg" style="background:${c}">${count===2 ? (i===0?'色 A':'色 B') : (i+1)}</div>`).join('')}
        <div class="scr-seg scr-ref" data-kind="post"><div class="scr-seal">END</div><span class="scr-ref-label">透明 / END</span></div>
      </div>`;
  }

  function deck(id,count,title,cls){
    return `<div class="scr-panel scr-deck ${cls}">
      <div class="scr-meta"><h3>${title}</h3><p>フィルム構成：<strong>${count}色</strong></p><p>1色あたり：約 <strong>${(256/count).toFixed(count===2?0:1)}</strong> step</p><p>${count===2?'大きく動かして2色を行き来':'現在色の前後を含め、約3色が見える'}。</p></div>
  ...
  function html(){
    return `<div class="scr-root">
      <div class="scr-panel scr-left">
        ...
        <button class="scr-power" id="scr-power">POWER OFF / CAL START</button>
      </div>
      <div class="scr-main">${deck('2',2,'2色スクローラー','red')}${deck('24',24,'24色スクローラー','green')}</div>
      <div class="scr-panel scr-bottom"><div class="scr-cal"><h3>キャリブレーション（基準出し）とは？</h3><p>電源投入時、透明フィルム上の START / END シールを透過センサーで検出し、実際の移動範囲を機械が覚えます。</p><div class="scr-steps">
        <div class="scr-stepcard" data-phase="1"><b>1 電源ON</b>CAL開始</div><div class="scr-stepcard" data-phase="2"><b>2 STARTへ</b>横移動</div><div class="scr-stepcard" data-phase="3"><b>3 START検出</b>センサー通過</div><div class="scr-stepcard" data-phase="4"><b>4 ENDへ</b>反対端へ</div><div class="scr-stepcard" data-phase="5"><b>5 END検出</b>センサー通過</div><div class="scr-stepcard" data-phase="6"><b>6 READY</b>DMX制御へ</div>
      </div></div><div class="scr-sense-note"><h3>START / END シールと透過センサー</h3><div class="scr-legend"><span class="scr-demo-transparent"></span>透明フィルム＝光が通る</div><div class="scr-legend"><span class="scr-demo-seal"></span>シール＝光を遮る</div><div>シールがセンサー位置を横切る瞬間を検出して、START と END を決める。</div><div class="scr-status" id="scr-status">POWER OFF</div></div></div>
    </div>`;
  }
  ...
  function inject(){
    ...
    if (typeof explanationData !== 'undefined') explanationData.scroller = '<strong>【Scroller：8-bitでフィルムの位置を決める】</strong><br>左のフェーダーを動かすと、同じDMX 0〜255で2色と24色のフィルムが横に移動します。24色側では現在色の前後を含めて約3色が見え、フィルムが連続して送られていることが分かります。<br><strong>【Calibration：START / ENDをセンサーで探す】</strong><br>POWERを入れると、透明フィルムに貼られたSTART / ENDシールを透過センサーで検出し、実際の移動範囲を決めます。';
    bind(tab,panel);
    render();
  ...
  function bind(tab,panel){
    ...
    track.addEventListener('pointerdown',e=>{S.dragging=true;track.setPointerCapture?.(e.pointerId);setFrom(e)});
    track.addEventListener('pointermove',e=>{if(S.dragging)setFrom(e)}); track.addEventListener('pointerup',()=>S.dragging=false); track.addEventListener('pointercancel',()=>S.dragging=false);
  ...
  function activate(tab,panel){
    ...
    if (typeof modules !== 'undefined' && typeof activeModuleKey !== 'undefined' && modules[activeModuleKey] && modules[activeModuleKey].destroy) modules[activeModuleKey].destroy();
    document.querySelectorAll('.main-tab-btn').forEach(t=>t.classList.remove('active')); tab.classList.add('active');
    document.querySelectorAll('.stage-view-panel.fade-type').forEach(p=>p.classList.remove('active')); panel.classList.add('active');
    document.getElementById('network-carousel-window')?.classList.remove('active');
    ...
    if (typeof activeModuleKey !== 'undefined') activeModuleKey='scroller';
    const help=document.getElementById('help-text-target'); if(help) help.innerHTML=(typeof explanationData!=='undefined'&&explanationData.scroller)?explanationData.scroller:'Scroller';
    render();
  }
  ...
  function togglePower(){
    ...
    if(S.power){S.power=false;S.ready=false;S.phase='POWER OFF';S.detectedStart=false;S.detectedEnd=false;clearTimeout(S.animTimer);render();return;}
    S.power=true;S.ready=false;S.detectedStart=false;S.detectedEnd=false;runCalibration();
  }

  function runCalibration(){
    S.calibrating=true; const seq=[
      {p:1,phase:'CAL START',pos:'center',ms:500},
      {p:2,phase:'MOVING TO START',pos:'startApproach',ms:900},
      {p:3,phase:'START SEAL DETECTED',pos:'start',ms:700,start:true},
      {p:4,phase:'MOVING TO END',pos:'endApproach',ms:1300},
      {p:5,phase:'END SEAL DETECTED',pos:'end',ms:700,end:true},
      {p:6,phase:'READY',pos:'normal',ms:500,ready:true}
    ]; let i=0;
    const next=()=>{ const x=seq[i++]; if(!x){S.calibrating=false;S.ready=true;S.phase='READY';render();return;} S.phase=x.phase;S.calPos=x.pos; if(x.start)S.detectedStart=true;if(x.end)S.detectedEnd=true;if(x.ready){S.ready=true;S.calibrating=false;} S.calStep=x.p;render();S.animTimer=setTimeout(next,x.ms);}; next();
  }

  function normalOffset(count,windowW){
    const segW=count===2?windowW*.50:windowW*.34; const total=(count+2)*segW; const idx=1+(S.dmx/255)*(count-1); return windowW*.5-(idx+.5)*segW;
  }
  function calibrationOffset(count,windowW,pos){
    const segW=count===2?windowW*.50:windowW*.34; const sensorX=windowW*.5; const last=count+1;
    let idx=1+(S.dmx/255)*(count-1); if(pos==='startApproach')idx=.18;if(pos==='start')idx=.5;if(pos==='endApproach')idx=count+.72;if(pos==='end')idx=count+1.5;if(pos==='center')idx=1; if(pos==='normal')return normalOffset(count,windowW); return sensorX-(idx+.5)*segW;
  }

  function updateFilm(id,count){
    const win=document.getElementById(`scr-window-${id}`),film=document.getElementById(`scr-film-${id}`); if(!win||!film)return; const w=win.clientWidth||600; const segW=count===2?w*.50:w*.34; film.querySelectorAll('.scr-seg').forEach(s=>s.style.width=`${segW}px`); const off=S.calibrating||S.calPos?calibrationOffset(count,w,S.calPos||'normal'):normalOffset(count,w); film.style.transform=`translateX(${off}px)`;
    document.getElementById(`scr-axis-label-${id}`).textContent=S.calibrating?S.phase:`現在位置 DMX ${S.dmx}`;
    const sensor=document.getElementById(`scr-sensor-${id}`); sensor.classList.toggle('detect',S.phase.includes('DETECTED'));
  }

  function render(){
    if(!document.getElementById('scr-root') && !document.querySelector('.scr-root')) return;
    const pct=Math.round(S.dmx/255*100); const bin=S.dmx.toString(2).padStart(8,'0').replace(/(.{4})/g,'$1 ').trim();
    ...
    const knob=document.getElementById('scr-knob'),fill=document.getElementById('scr-fill'); const y=S.dmx/255*100; knob.style.bottom=`calc(${y}% - 25px)`;fill.style.height=`${y}%`;
    updateFilm('2',2);updateFilm('24',24);
    const pb=document.getElementById('scr-power');pb.classList.toggle('on',S.power);pb.textContent=S.power?'POWER ON / CAL AGAIN':'POWER OFF / CAL START';
